...
sealed class LoadableResult<R>(open val payload: R? = null) {
    ...
    data class Failure<R>(
        ...
        override val payload: R?,
        val error: String? = throwable.localizedMessage,
    ) : LoadableResult<R>(payload)
    ...
    companion object {
        ...
        fun <R> failure(
            throwable: Throwable,
            payload: R? = null,
            error: String? = throwable.localizedMessage,
    ...
    inline fun <C> fold(
        ifLoading: (payload: R?) -> C, ifFailure: (t: Throwable, payload: R?) -> C, ifSuccess: (R, payload: R?) -> C
    ): C = when (this) {
    ...
    inline fun doOnFailure(operation: (String?) -> Unit) {
